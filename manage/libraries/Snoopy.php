...
class Snoopy
{
    ...
    function fetchid($URI)
    {
        if ($this->fetch($URI) !== false) {
            ...
            if (is_array($this->results)) {
                for ($x = 0; $x < count($this->results); $x++)
                    $this->results[$x] = $this->_stripid($this->results[$x]);
            } else
                $this->results = $this->_stripid($this->results);
    ...
    function _stripid($document)
    {
        preg_match_all("'<\s*(\b\w*\b)[^\<\>]+?\s*?id\s*=\s*			# find <a href=
                        ([\"\'])?					# find single or double quote
                        (?(2) (.*?)\\2)		# if quote found, match up to next matching
